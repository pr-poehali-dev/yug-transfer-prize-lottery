...
def handle_telegram(update: dict):
    ...
    try:
        ...
        if msg:
            ...
            # В группе команды подписки/статуса не работают — отвечаем только в личке.
            # Деплинк /start accept_<id> (приём заказа) обрабатывается ниже отдельно.
            if chat_type in ('group', 'supergroup'):
                t = text.strip()
                is_sub_cmd = (t.startswith('/podpiska') or t.startswith('/подписка')
