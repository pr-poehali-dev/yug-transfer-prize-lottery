"""Telethon-логин user-аккаунта для публикации сторис в канал @ug_transfer_pro.
POST ?action=send_code   — отправить код на номер
POST ?action=verify_code — подтвердить код, получить session
POST ?action=verify_2fa  — подтвердить пароль 2FA если включён
GET                       — статус сессии
"""
import os
import json
...
def get_state() -> dict:
    conn = db()
    cur = conn.cursor()
    cur.execute(f"SELECT phone, phone_code_hash, session_string, logged_in, user_info FROM {SCHEMA}.tg_user_session WHERE id=1")
    r = cur.fetchone()
...
def save_state(phone=None, phone_code_hash=None, session_string=None, logged_in=None, user_info=None):
    ...
    cur.execute(f"UPDATE {SCHEMA}.tg_user_session SET {', '.join(parts)} WHERE id=1")
...
def handler(event: dict, context) -> dict:
    """Логин user-аккаунта Telegram через Telethon. Возвращает статус сессии."""
    ...
    try:
        ...
        if action == 'verify_code':
            ...
            result = loop.run_until_complete(verify_code(code))
            return resp(200, result)
        if action == 'verify_2fa':
            password = body.get('password', '')
            result = loop.run_until_complete(verify_2fa(password))
            return resp(200, result)
        if action == 'logout':
            result = loop.run_until_complete(logout())
            return resp(200, result)
