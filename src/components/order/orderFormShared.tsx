...
import useTariffCalc, { applySelectedTariffFromCache, clearTariffPriceCache, renderTariffCards, syncOrderPriceField } from "@/hooks/useTariffCalc";
...
export function useOrderForm(): OrderFormState {
  ...
  useEffect(() => {
    ...
    const ROUTE_FIELDS = ["place_start", "place_end", "Date", "Time", "count_peeple", "count_bags"];
    // Доп. опции цену маршрута не меняют — это фиксированные надбавки,
    // поэтому кэш НЕ сбрасываем, а просто пересчитываем итог с доплатами.
    const EXTRA_FIELDS = ["orderBabyChair", "orderBuster", "orderPet"];
    const onRouteChange = (e: Event) => {
      const name = (e.target as HTMLElement)?.getAttribute?.("name") || "";
      if (ROUTE_FIELDS.includes(name)) clearTariffPriceCache();
      if (EXTRA_FIELDS.includes(name)) {
        renderTariffCards();
