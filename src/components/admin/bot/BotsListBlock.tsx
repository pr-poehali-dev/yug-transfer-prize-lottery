...
import { BOTS, BotInfo, colorMap } from "./botTypes";

interface BotsListBlockProps {
  token: string;
  show: boolean;
}

export function BotsListBlock({ token, show }: BotsListBlockProps) {
  ...
  useEffect(() => {
    if (show) BOTS.forEach(b => fetchBotInfo(b.id, b.url));
  }, []);
